            ...
            <b-col cols="12">
              <div class="upload-example">
                ...
                <b-col cols="6" offset="3">
                  <div class="img-name">
                    <label class="mr-2 photo-name-label"
                      >{{ $t("posts.photo name") }}
                    </label>
                    <b-form-group>
                      <b-form-input
  ...
  data() {
    return {
      languages: [
        ...
        { langText: "Fransizca 🇫🇷", langCode: "fr" },
        { langText: "Arapca 🇸🇦", langCode: "ar" },
      ...
      images.split("~").forEach((image) => {
        if (image != "")
          this.sliderImages.push(
            process.env.VUE_APP_SERVER_URL + "/images/" + image + ".jpeg"
          );
      });
    }
      ...
      if (vm.toggleEditImage && canvas) {
        ...
        canvas.toBlob((blob) => {
          vm.blobToBase64(blob).then((res) => {
            ...
            axios
              .post(
                process.env.VUE_APP_SERVER_URL + "/create_post/",
                formData,
                {
                  headers: {
    ...
    uploadVideo(event) {
      ...
      if (input.files && input.files[0]) {
        ...
        const request = axios.post(
          process.env.VUE_APP_SERVER_URL + "/upload_video",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
