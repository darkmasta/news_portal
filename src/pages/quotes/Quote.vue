      ...
      <b-col cols="9">
         <div class="d-flex justify-content-between">
          <div class="page-header">
            <h4>View/Update Quote</h4>
            <span>Hello {{user.firstName}}, welcome back!</span>
          </div>
          <div>
            <b-btn variant="secondary icon-btn" pill @click=""><i class="fas fa-long-arrow-alt-left"></i></b-btn>
        ...
        <b-card class="quote-card">
          ...
          <b-row no-gutters>
            <b-col>
              ...
              <b-card class="quote-price-item mt-2" v-for="(car, index) in cars" :key="index" @click="selectCar(index)">
                <b-row no-gutters>
                  <b-col cols="1">
                  ...
                  <b-col cols="10">
                    <b-row no-gutters>
                      ...
                      <b-col cols="6" class="mt-2">
                        <span v-if="editCar == index"><b-input class="car-input" placeholder="margin" v-model="editedMargin"></b-input></span>
                        <span v-if="editCar != index">{{car.margin}}</span>
                      </b-col>
                    </b-row>
                  </b-col>
                  <b-col cols="1">
                    <b-btn variant="secondary icon-btn rounded-pill" size="sm"><i class="far fa-edit" @click="selectCar(index)"></i></b-btn>
                    <b-btn variant="secondary icon-btn rounded-pill" size="sm"><i class="fa fa-trash" @click="removeCar(index)"></i></b-btn>
                    <b-btn v-if="editCar == index" variant="secondary icon-btn rounded-pill" size="sm"><i class="fa fa-save" @click="saveCar(car.margin, index)"></i></b-btn>
                  </b-col>
                </b-row>
              </b-card>
        ...
        <b-row no-gutters>
          <b-col>
            <div class="d-flex justify-content-between mt-4">
              <b-btn variant="secondary" class="font-weight-bold save-quote" @click="createQuote">Update Quote</b-btn>
              <b-btn variant="primary" class="font-weight-bold save-quote" @click="createOrder()">Book It Now</b-btn>
            </div>
  ...
  created() {
    ...
    axios.post(process.env.VUE_APP_SERVER_URL + '/get_quote_by_id', {data})
            .then(response => { 
                ...
                this.$store.commit('setSelectedQuote', quoteData)
               //  userData.cars.shift()
  ...
  methods: {
    ...
    createQuote() {
     ...
     console.log(data)
      ...
      axios
        .post(process.env.VUE_APP_SERVER_URL + "/update_quote/", { data })
        .then((response) => {
    ...
    saveCar(margin, index) {
      var vm = this
      this.$nextTick(() => {
        vm.cars[index].margin = Number(this.editedMargin)
        vm.cars[index].selected = false 
        vm.cars[index].edit = false 
